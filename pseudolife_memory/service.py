...
class MemoryService(DreamOps):
    ...
    def episode_end(self, episode: str | None = None) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if closed is not None and closed.parent_id is None:
                # No sub-episode was open, so this closed the session root.
                self._end_client_session_locked(session_id, closed, "end")
            self._persist_episodes()
