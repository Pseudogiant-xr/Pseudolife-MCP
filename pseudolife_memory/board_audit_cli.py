...
Both read the bank directly through ``PSEUDOLIFE_MCP_DATABASE_URL`` in a
read-only snapshot, so they need the database owner's credentials, never a
bearer token, and are safe to run beside a live daemon. There is deliberately
...
Exit status: 0 success, 1 the chain failed verification, 2 nothing could be
checked (bad arguments, no database, no audit log).
"""
...
def _connect():
    ...
    if not dsn:
        raise AuditCliError("PSEUDOLIFE_MCP_DATABASE_URL is required (the bank's database URL)")
    import psycopg
    conn = psycopg.connect(dsn, connect_timeout=5)
    try:
        # One read-only, repeatable-read snapshot: a daemon appending while
        # this runs cannot tear the chain being read.
        conn.read_only = True
...
def _verify(args) -> int:
    if args.input:
        ...
        with _connect() as conn, closing(audit_events(conn)) as rows:
...
def main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Export or verify the agent board's audit log (operator-only, read-only). "
                    "Reads PSEUDOLIFE_MCP_DATABASE_URL. The output holds message bodies: "
                    "keep it private.")
    ...
    try:
        ...
        # The reader stopped early (``export | head``); nothing is wrong
        # with the bank, and the interpreter must not complain at exit.
