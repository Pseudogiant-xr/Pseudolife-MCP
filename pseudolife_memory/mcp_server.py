...
def memory_episode_start(
    ...
    search. Pass ``episode`` (your session handle from the briefing) to
    anchor it to YOUR session when several run concurrently.
    ``memory_episode_end`` closes it and pops back.
    """
...
def memory_episode_end(episode: str | None = None) -> dict[str, Any]:
    """Close the current open sub-episode and pop back to its parent. Pass
    ``episode`` (your session handle) to pop only within your own session's
    subtree; the session root is never closed.
    """
