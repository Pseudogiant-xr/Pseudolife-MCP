...
该智能体在工作过程中会逐条存入声明(`memory_store`、`memory_fact_set`)。在会话之间,**dream** 会把记忆流蒸馏为规范事实、图谱关系与过程性经验教训。在装有会话钩子的环境中(Claude Code 插件、安装脚本写入 Claude Code `settings.json` 的钩子,或经过验证的 Codex 钩子),每次会话开始时,简报都会注入记忆中尚不确定的部分、过往工作的经验教训,以及你上次停下的地方。两条命令的快速开始不会安装任何钩子,安装脚本也不会为 Gemini CLI 或其他 MCP 智能体接入钩子,因此这些配置不会获得简报;不过,MCP 服务器自带的简短指令仍会要求智能体在任务开始时搜索记忆。检索会将联想存储上的语义搜索与规范事实库结合起来,使已更正的答案胜过过时的答案。
