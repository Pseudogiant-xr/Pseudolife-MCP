...
사실, 그래프 관계, 절차적 교훈으로 압축합니다. 세션 훅이 설치된 환경(Claude Code
플러그인, 설치 스크립트가 작성하는 Claude Code `settings.json` 훅, 또는 검증된
Codex 훅)에서는 매 세션 시작 시 브리핑이 메모리가 확신하지 못하는 부분, 과거
작업에서 얻은 교훈, 그리고 지난번에 멈춘 지점을 주입합니다. 명령 두 줄짜리 빠른
시작은 훅을 전혀 설치하지 않으며, 설치 스크립트도 Gemini CLI나 그 밖의 MCP
에이전트에는 훅을 연결하지 않으므로 이러한 구성에서는 브리핑이 제공되지 않습니다.
다만 MCP 서버 자체의 짧은 지침은 여전히 작업 시작 시 메모리를 검색하도록
에이전트에 요청합니다.
