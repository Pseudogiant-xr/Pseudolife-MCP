...
| **Source exclusion from consolidation** (`memory.dream.exclude_sources`, default `consolidation` / `reflection` / `status` / `log`) | Keeps high-volume, low-value chatter out of the dream's input entirely — those entries stay searchable but are never mined for facts or graph edges, shrinking the surface that can reach canonical authority | On |
...
| **`memory_forget` + engram links** | Remediation. Deletion is the only reliable remedy; the links tell you what else to retire | On |
