...
3. **Standing instructions file** — the full memory-loop block
   ...
   why the installer recommends the append there (and only prompts — it
   never writes a standing file without consent, and never in a
   non-interactive run).
...
`-s user` matters — Gemini defaults to *project* scope. Gemini CLI has no
