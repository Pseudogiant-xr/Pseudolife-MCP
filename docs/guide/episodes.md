...
SessionEnd, shim exit or an episode end that closes the root; `idle` for
the reaper; cleared when the session registers again or a store or handle
reopens its root), the startup memory-policy variant the hook assigned
