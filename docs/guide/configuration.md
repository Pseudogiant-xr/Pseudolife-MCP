...
The plugin's SessionStart hook therefore records the shim's digest key once
per Claude Code process, in `claude-<CLAUDE_PID>.host` beside the digests,
and the prompt hook reads through that record. `CLAUDE_PID` is the Claude Code
process id, which Claude Code v2.1.214 and later export to hooks; older
versions keep the per-session key. After `/clear`, compaction or a resume,
the current digest prints once more. A `--continue` launch whose shim got the
