...
a day longer (a cut takes a whole prefix, so after the daemon's clock steps
backwards it can take rows younger than the window by up to that step), with
at most one cut a day. The cut is always a prefix, recorded
