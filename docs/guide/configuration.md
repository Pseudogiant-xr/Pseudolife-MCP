...
start instead of leaving a short bank behind; while it is unfinished
`/health` reports `status: "degraded"` with a `migration_partial` field.
Leave the original `.pt` files in place until it completes — the resume
reads them.
