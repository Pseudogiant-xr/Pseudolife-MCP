...
It cuts on UTC day boundaries, so an event stays at least the window and at most
a day longer, with at most one cut a day. The cut is always a prefix, recorded
...
Record `head_seq:head_hash` and `head_created_at` from a `verify` somewhere
outside the bank, and later run `verify --expect-head SEQ:HASH`. That catches
the first two. For the third, `head_pruned` on a head created less than
`audit_retention_days` ago, or a `start_cut` whose `retention_days` is not the
window you configured, means rows went that retention would have kept. For
history you must be able to prove, keep periodic `--out` exports (privately)
and check them with `verify --input`. The log records mutations made through
