...
address again. Any lease it held must also have been gone for an hour, so a
daemon restart or a host sleep cannot retire a live shim's address, and if a
state-less address is ever retired its adapter registers a fresh one instead
of stopping.
...
`memory_message`, and names the way out in the error. A Claude Code session,
including one in the Desktop app's Code tab, makes those calls on its own
per-session server; `memory_agents(action="list")` still works through the
shared one. A
