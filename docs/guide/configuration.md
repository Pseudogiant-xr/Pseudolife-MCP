...
for an A/A arm. The arm is logged by the daemon and can be recomputed from
`episodes.session_key` for a window whose arm list is known; sessions that
reach the hook without a session id keep `variant`. A custom
