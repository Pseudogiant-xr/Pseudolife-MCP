...
`paths`, supporting `texts`, and `iterations`. `entities`/`edges`/`texts`
are each capped (10/15/6 by default — a 2026-08-21 audit measured an
uncapped 3-hop query at 93.7 KB, mostly from 53 entities and 45 full
entry texts) to the highest-relevance items in the traversal's own
existing order — no separate ranking pass. With `verbose=False` (the
default), `texts` are also truncated to a preview length; `verbose=True`
