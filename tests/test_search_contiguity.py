...
def test_contiguity_off_by_default_is_identical():
...
def test_contiguity_expands_hits_with_temporal_neighbors():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as d:
        svc = MemoryService(data_dir=d)
        _seed_sequence(svc)
...
def test_contiguity_neighbors_never_duplicate_direct_hits():
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as d:
        svc = MemoryService(data_dir=d)
        _seed_sequence(svc)
...
def test_temporal_neighbors_scope_and_tie_break():
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as d:
        svc = MemoryService(data_dir=d)
        svc.store("warmup", source="user")  # forces CMS init
        cms = svc._cms
