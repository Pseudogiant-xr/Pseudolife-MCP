"""Merge-proposal veto rules (graph_review.merge_veto) — pure, no DB.

Two rules, both measured against the 2026-08-11 triage ground truth
(tests/fixtures/merge_triage_replay_20260811.json, 153 human/agent-judged
proposals):
...
# ── wiring + re-orientation (PG-backed) ───────────────────────────────────
...
def test_deep_dream_filters_vetoed_merge_candidates(svc):
    ...
    # token-mention entries (entity_context_vectors' fallback scan), with
    # the broader name mentioned in the narrower name's entries too, so the
    ...
    for text in (
        ...
        svc.store(text, source="veto-wiring-test")
