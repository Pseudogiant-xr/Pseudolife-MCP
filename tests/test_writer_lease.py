...
def test_mail_send_after_writer_handover_out_ranks_new_bank_history(
        pg_conn, pg_url, tmp_path):
    """A's live mailbox must not stamp from its stale clock after B wrote."""
    ...
    a._hlc = HybridLogicalClock(now_ms=lambda: 100)
    b = None
    try:
        a.cortex_write("svc", "port", "8080")
        ...
        b = MemoryService(data_dir=tmp_path / "b", database_url=pg_url)
        b._hlc = HybridLogicalClock(now_ms=lambda: 10000)
        b.cortex_write("svc", "port", "9090")
        high = pg_conn.execute(
            "SELECT hlc_phys, hlc_logical FROM facts "
            "WHERE entity=%s AND attribute=%s AND status='current'",
            ("svc", "port"),
        ).fetchone()
        pg_conn.commit()
        b._storage.close()
        ...
        pg_conn.commit()
        assert tuple(map(int, stamp.split(":"))) > tuple(high)
        assert a.cortex_lookup("svc", "port")["value"] == "9090"
    finally:
        if b is not None and b._storage is not None:
            b._storage.close()
...
def test_mail_send_refuses_while_another_writer_holds_the_bank(
    ...
    try:
        ...
        b.set_meta("handover-written", True)

        with pytest.raises(ValueError, match="coordination_unavailable"):
            dispatch(a, "send", {"to": agent["agent_id"],
                                 "text": "must wait for lease",
                                 "request_id": "held-writer"},
                     headers=creds, principal=principal)
        assert pg_conn.execute(
            "SELECT count(*) FROM coordination_messages WHERE request_id='held-writer'"
        ).fetchone()[0] == 0
        pg_conn.commit()
    finally:
        if b is not None:
...
@pytest.mark.parametrize("bad_epoch", [None, "broken"])
def test_mail_send_refuses_missing_or_malformed_writer_epoch(
        pg_conn, pg_url, tmp_path, bad_epoch):
    from pseudolife_memory.coordination import dispatch
    from pseudolife_memory.service import MemoryService
    ...
    service.config.coordination.allowed_principals = [principal]
    try:
        service.cortex_write("svc", "port", "8080")
        agent = dispatch(service, "register", {}, headers={}, principal=principal)
        creds = {"x-pl-agent": agent["agent_id"],
                 "x-pl-agent-key": agent["credential"]}
        if bad_epoch is None:
            pg_conn.execute("DELETE FROM meta WHERE key='writer_lease_epoch'")
        else:
            pg_conn.execute("UPDATE meta SET value=%s WHERE key='writer_lease_epoch'",
                            (psycopg.types.json.Jsonb(bad_epoch),))
        pg_conn.commit()
        with pytest.raises(ValueError, match="coordination_unavailable"):
            dispatch(service, "send", {"to": agent["agent_id"],
                                       "text": "must be refused",
                                       "request_id": "bad-epoch"},
                     headers=creds, principal=principal)
        assert pg_conn.execute(
            "SELECT count(*) FROM coordination_messages WHERE request_id='bad-epoch'"
        ).fetchone()[0] == 0
        pg_conn.commit()
    finally:
        ...
        if mailbox is not None:
            mailbox.close()
        if service._storage is not None:
            service._storage.close()


def test_mail_send_refuses_corrupt_highwater_after_handover(
        pg_conn, pg_url, tmp_path):
    from pseudolife_memory.coordination import dispatch
    from pseudolife_memory.service import MemoryService
    ...
    try:
        ...
        creds = {"x-pl-agent": agent["agent_id"],
                 "x-pl-agent-key": agent["credential"]}
        _kill(pg_conn, service._storage.conn.info.backend_pid)
        other = PostgresStorage(pg_url)
        try:
            other.set_meta("coordination_hlc_highwater", "broken")
        finally:
            other.close()
        with pytest.raises(ValueError, match="invalid_request"):
            dispatch(service, "send", {"to": agent["agent_id"],
                                       "text": "must be refused",
                                       "request_id": "bad-highwater"},
                     headers=creds, principal=principal)
        assert pg_conn.execute(
            "SELECT count(*) FROM coordination_messages WHERE request_id='bad-highwater'"
        ).fetchone()[0] == 0
        pg_conn.commit()
    finally:
        mailbox = getattr(service, "_coordination_storage", None)
