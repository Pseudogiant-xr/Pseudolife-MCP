...
class Board:
    ...
    def __init__(self, agents, *, statuses=None):
        self.agents = dict(agents)          # name -> principal
        self.statuses = dict(statuses or {})
        self.messages = []
        self.extra = []                     # (offset, event, name, payload)
    ...
    def event(self, offset, event, name, payload):
        self.extra.append((offset, event, name, payload))
    ...
    def legacy(self, exported_at):
        agents = [{"agent_id": aid(n), "principal": p, "label": f"agent-label-{n}",
                   "status": self.statuses.get(n, ""), "project": "project", "task": "task",
        ...
        messages = [{"message_id": m["id"], "sender_agent_id": aid(m["sender"]),
                     ...
                     "text": m["text"], "reply_to": m["reply_to"], "project": "project",
                     "task": "task", "request_id": f"r-{m['id']}", "hlc": "1:0",
    ...
    def audit(self):
        ...
        for n, p in self.agents.items():
            pending.append((T0 - 600, "register", n, None, None, {
                "label": f"agent-label-{n}", "project": "project", "task": "task", "episode": "",
                "status": self.statuses.get(n, ""), "capabilities": {}, "wake_enabled": False}))
        for m in self.messages:
            ...
            if m["kind"] is not None:
                payload["kind"] = m["kind"]
            pending.append((T0 + m["sent"], "send", m["sender"], m["recipient"], m["id"], payload))
            if m["acked"] is not None:
                ...
                pending.append((T0 + m["acked"], "ack", m["recipient"], m["recipient"], m["id"],
                                sender))
        for offset, event, name, payload in self.extra:
            pending.append((T0 + offset, event, name, None, None, payload))
        pending.sort(key=lambda e: e[0])
        rows = []
        for seq, (at, event, name, recipient, message_id, payload) in enumerate(pending, 1):
            daemon = name is None
            rows.append({"seq": seq, "event": event, "actor": "daemon" if daemon else "agent",
                         "principal": "" if daemon else self.agents[name],
                         "agent_id": "" if daemon else aid(name),
                         "recipient_agent_id": None if recipient is None else aid(recipient),
                         "project": "" if daemon else "project", "task": "" if daemon else "task",
...
def sentinel_board():
    names = {"c": "claude-code", "w1": "claude-code", "w2": "claude-code", "w3": "claude-code",
             "x1": "codex"}
...
def test_a_principal_that_is_not_a_plain_name_is_pseudonymized(tmp_path):
    b = Board({"a": "claude-code", "b": "Sentinel User <sentinel-user@example.com>"})
    b.send("a", "b", 0, 5)
    report = run(write_legacy(tmp_path, b))
    text = json.dumps(report)
    assert "sentinel-user" not in text and "example.com" not in text
    assert set(report["metrics"]["ack_latency"]["overall"]) == {"all", "principal-1"}
