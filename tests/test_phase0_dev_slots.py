...
def test_of_form():
    s = _slots("the host of the relay server is 192.168.0.130")
    assert ("relay server", "host", "192.168.0.130") in s


def test_colon_form_with_entity():
    s = _slots("redacted fallback host: 192.168.0.130:1236")
    assert ("redacted fallback", "host", "192.168.0.130:1236") in s
...
def test_code_fence_skipped():
    text = "Set it up like this:\n```\nredacted fallback host: 10.0.0.1\n```"
