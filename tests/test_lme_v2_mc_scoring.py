"""The V2 smoke's multiple-choice scorer must not read prose as an answer.

``score_mc``'s no-box fallback used to accept any standalone ``[A-Ha-h]``
token, upper-cased. In an answerer that ran out of tokens mid-reasoning —
the dominant unboxed shape in every committed lme-v2 artifact — that
matches the English article "a" and scores the row as answer **A**.
Measured on ``evals/results/lme-v2-smoke-slice2.jsonl`` (2026-08-25 audit,
issue #173): of 105 MC arm-answers 57 carry no box, and the old fallback's
matched token was the article "a" 40 times against 4 real uppercase
letters — all of the latter option *enumerations* ("*   A. Service
Catalog"), never a stated answer.

So the fallback is now uppercase-only AND anchored: the whole response is
the letter, or the letter follows an explicit answer marker. Anchoring on
a bare "Option X" was measured and rejected — option enumerations are the
noise class itself.
"""
import sys
