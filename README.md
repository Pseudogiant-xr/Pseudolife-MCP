...
The **Claude Code plugin** is a third install that does not move with the
daemon either: Claude Code runs it from a cache that only changes when you
ask. After a daemon update, refresh it inside Claude Code and start a new
session:
...
The three tell on each other: `/health` reports the daemon's
`version`, the session briefing opens with a one-line notice when the
plugin's version differs from it (naming the command above or the daemon
redeploy, whichever side is behind), the shim says so on stderr and ahead
of its instructions, and `pseudolife-mcp doctor` reports `version_mismatch`.
