...
- **Three worked examples in the extraction-prompt lineage were written from
  ...
  in 82 of 90 pre-change oracle runs and wrong in all 58 `_s` runs of either
  era, so the example is not load-bearing. The defect is the method — a rule
  fitted to a diagnosed benchmark item and gated on the slice holding it —
  not the numbers.
