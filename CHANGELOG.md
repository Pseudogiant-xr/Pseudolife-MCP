...
- `ops/setup-codex-hooks.py` could hang on a busy Windows machine for as
  long as a killed hook lingered (two test runs were still stuck when
  stopped after 7 and 10 minutes). Codex kills
  a hook that outruns its budget, and a PowerShell hook killed while still
  starting up could stay stuck mid-exit, holding an inherited copy of the
  Codex app-server's output pipe; setup waited on that pipe with no limit.
  It now gives the pipe two seconds after the app-server exits and moves
  on, so setup finishes and reports the verification check that failed.
