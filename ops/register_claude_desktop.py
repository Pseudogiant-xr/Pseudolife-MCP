...
third-party packages; its package imports (the credential-file writer and
...
* Desktop launches MCP servers with a SANITIZED environment (PATH plus a
  ...
  the literal is removed from the config. A configured or explicitly chosen
  usable file is preserved on rerun. Only a fresh installer default with no
  available credential may write a credentialless entry and exit 3; an
  unusable configured or explicit path is refused without rewriting config.
* The shim must be able to READ that file. Installers historically took the
  shim from PyPI, and every release through 0.15.0 reads only the literal
...
Usage:
    python ops/register_claude_desktop.py --command /abs/path/pseudolife-mcp
        ...
        [--default-token-file /abs/path/claude-desktop.token]
        [--token-from-env VAR_NAME] [--tokens-from-env VAR_NAME]
        [--config PATH] [--dry-run] [--skip-shim-check]
...
PROBE_CREDENTIAL_ENV = frozenset((
    LITERAL_TOKEN_KEY,
...
def shim_reads_token_file(command: str, *, timeout: float = SHIM_PROBE_TIMEOUT,
    ...
    installer's terminal. Canonical credential variables are always removed;
    ``scrub_env`` names additional caller-selected token sources the child
    must not inherit. The UTF-8 pin keeps the em dashes in the usage text
    from failing the child on a narrow console code page."""
    blocked_env = PROBE_CREDENTIAL_ENV.union(scrub_env)
    env = {key: value for key, value in os.environ.items()
           if key not in blocked_env}
...
def _refuse_old_shim(command: str) -> int:
    print(
        ...
        f"`pipx install --force {REPO_ROOT}` (or `pip install -U {REPO_ROOT}`) — then "
...
def _desktop_token_from_map(raw: str | None) -> str:
    """Return the sole token mapped to the Desktop principal."""
    root = str(REPO_ROOT)
    sys.path.insert(0, root)
    try:
        ...
        raise ConfigError(
            f"cannot import pseudolife_memory.principals from {REPO_ROOT}; "
            "run this script from a Pseudolife-MCP checkout") from exc
    matches = [token for token, principal in parse_token_map(raw).items()
               if principal == DEFAULT_WRITER_ID]
    if len(matches) != 1:
        raise ConfigError(
            "the daemon token map must contain exactly one usable credential "
            f"for principal {DEFAULT_WRITER_ID!r}; found {len(matches)}")
    return matches[0]


def plan_credential(existing_env: Mapping[str, str], token_file: str | None,
                    token: str | None, *, dry_run: bool,
                    preserve_existing: bool = False) -> tuple[str | None, bool, list[str]]:
    ...
    if preserve_existing:
        ...
        try:
            ...
            raise ConfigError(
                ...
                "fix it or pass --token-file with another usable path") from exc
        prefix = "would use" if dry_run else "using"
        notes.append(f"token file: {prefix} the existing {token_file}")
        if literal:
            notes.append(f"removed the superseded literal {LITERAL_TOKEN_KEY} from the planned entry")
        return token_file, bool(literal), notes
    if source:
        if Path(token_file).exists():
            creds = _credentials()
            try:
                current = creds.CredentialProvider(path=token_file).snapshot().token
            except (creds.CredentialError, OSError, UnicodeError):
                current = None  # a supplied source can safely repair the file
            if current == source:
                prefix = "would use" if dry_run else "using"
                notes.append(f"token file: {prefix} the existing {token_file}")
                return token_file, bool(literal), notes
...
def main(argv: list[str] | None = None) -> int:
    ...
    token_paths = parser.add_mutually_exclusive_group()
    token_paths.add_argument("--token-file", default=None,
                        help="absolute path of the private file holding the daemon "
                             "bearer; explicitly replaces any configured path")
    token_paths.add_argument("--default-token-file", default=None,
                        help="installer default used only when the existing entry has "
                             "no token-file path")
    parser.add_argument("--token-from-env", default=None, metavar="VAR",
                        help="name of an environment variable holding the token to "
                             "write into the selected token file (the installer sets one; the "
                             "value is never printed)")
    parser.add_argument("--tokens-from-env", default=None, metavar="VAR",
                        help="name of an environment variable holding the daemon's "
                             "token:principal map; exactly one claude-desktop "
                             "credential is required (values are never printed)")
    ...
    for candidate in (args.token_file, args.default_token_file):
        if candidate and not _is_absolute(candidate):
            print(f"error: token file must be an absolute path (got {candidate!r})",
                  file=sys.stderr)
            return EXIT_REFUSED
    try:
    ...
    try:
        ...
        existing_token_file = existing_env.get("PSEUDOLIFE_MCP_TOKEN_FILE")
        requested_file = (args.token_file or existing_token_file
                          or args.default_token_file)
        if requested_file and not _is_absolute(requested_file):
            raise ConfigError(
                f"the existing token file path is not absolute ({requested_file!r}); "
                "pass --token-file with a usable absolute replacement")

        # Probe the path that selection actually chose, including a configured
        # custom path preserved over the installer's default. This stays after
        # the read-only config load but before either credential or config writes.
        if requested_file:
            if args.skip_shim_check:
                print("shim check: skipped (--skip-shim-check); the entry assumes "
                      f"{args.command} reads {TOKEN_FILE_MARKER}")
            else:
                scrub = tuple(name for name in (
                    args.token_from_env, args.tokens_from_env) if name)
        ...
        preserve_existing = bool(
            (existing_token_file and not args.token_file)
            or (args.token_file and Path(args.token_file).exists())
        )
        if preserve_existing:
            token_file, drop_literal, notes = plan_credential(
                existing_env, requested_file, None, dry_run=args.dry_run,
                preserve_existing=True)
        else:
            if not token and args.tokens_from_env:
                token = _desktop_token_from_map(os.environ.get(args.tokens_from_env))
            if token and not requested_file:
                raise ConfigError(
                    "a token source was supplied but there is no token-file path; "
                    "pass --token-file or --default-token-file")
            token_file, drop_literal, notes = plan_credential(
                existing_env, requested_file, token, dry_run=args.dry_run)
        if token_file is None and args.token_file:
            raise ConfigError(
                f"the explicit token file {args.token_file} does not exist and no "
                "credential was available to write it; existing config was preserved")
        if token_file is None and existing_token_file:
            raise ConfigError(
                f"the configured token file {existing_token_file} does not exist and "
    ...
    if args.dry_run:
        ...
        for note in notes:
            print(note)
        return _finish(no_credential, requested_file)
    if not changed:
